...
function tasksFor(day){
  ...
  // Organique
  const org = [
    {k:'energie', label:`Énergie ${p===1? '30m': p===2? '40m':'45m'}`, icon:'⚡', pillar:'org'},
    {k:'conscience', label:`Conscience ${p===1? '12m': p===2? '15m':'20m'}`, icon:'🔮', pillar:'org'},
    {k:'clarte1', label:'Clarté Cat.1', icon:'✨', pillar:'org'},
    {k:'clarte2', label:'Clarté Cat.2', icon:'✨', pillar:'org'},
...
function guideFor(day){
  const p=phaseOf(day);
  if(p===1) return `Socle: sommeil régulier, Clarté Cat.2 zéro, Cat.1 → Vert d’ici J7, Énergie/Conscience quotidiennes, micro‑expression.`;
...
function scheduleLocalNotifs(){
  ...
  add('Shift 90 — Matin','Check-in: Énergie, Conscience, Paye.', timeToNext(notif.morning));
